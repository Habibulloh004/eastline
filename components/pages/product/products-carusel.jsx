        ...
        {remainingImages.length > 0 && (
          <div className="w-full flex gap-4 mt-2">
            {remainingImages.map((image, index) => (
              <div onClick={() => handleClick(image)} key={index}>
                <CustomImage
                  src={image}
                  alt={item.name}
                  className="w-16 h-16 object-cover border-2 rounded-xl p-1 cursor-pointer"
                />
              </div>
            ))}
          </div>
        ...
        >
          ...
          <CarouselContent className="h-[300px] lg:h-full">
            {item.image.map((c, idx) => (
              <CarouselItem key={idx} className="bg-white rounded-md basis-full md:basis-[60%] ">
                <CustomImage
                  src={c}
                  alt={item.name}
