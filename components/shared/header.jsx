        ...
        <Container className="text-secondary">
          <ul className="flex items-center gap-2 md:gap-10">
            {navItems.map((item, idx) => {
              ...
              return (
                <li key={item.id} className="h-10 flex items-center justify-center">
                  {item.id == 2 ? (
                    <DropdownMenu>
                    ...
                    <Link href={`${item.path}`} className="py-2 px-3">{item.name}</Link>
                  )}
                </li>
              );
            })}
          ...
          <div className="hidden items-center gap-1 md:gap-5 lg:flex">
            <p>info@elt.uz</p>
            <a href="tel: +998909337880" className="flex items-center gap-2"><Phone size={16} />+998 90 933-78-80</a>
            <a href="tel: +998555108133" className="flex items-center gap-2"><Phone size={16} />+998 55 510-81-33</a>
          </div>
        </Container>
      </Container>
    </header>
